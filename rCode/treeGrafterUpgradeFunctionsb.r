...
read_panther <- function(x, tree.reader = ape::read.tree, ...) {
  # Reading the data-in
  x  <- readLines(x)
  x[1] = gsub('SAR/','SAR-',x[1]) # function doesn't handle slash correctly, so replacing with dash
  x[1] = gsub('Ev=UNK','Ev=2>2',x[1]) # Function doesn't correctly handle Unknowns, so replacing with something it can handle
  ...
  ans <- data.frame(
    ...
    type             = ifelse(dat[,3] == "0>1", "S",
                              ifelse(dat[,3] == "2>2","U", # for unknown case
...
unpackorthos = function(vec,pos = 1){
 ...
 # function to get mrca for pairs

mrcapairs = function(edgeList,node1,node2){
    # get the ancestors list of one node
    if(node1 == edgeList[1,1]) return(node1)
    if(node2 == edgeList[1,1]) return(node2)
    anc = c(edgeList[edgeList[,2]==node1,1])
    while(anc[length(anc)] != edgeList[1,1]){
        anc = c(anc,edgeList[edgeList[,2]==anc[length(anc)],1])
    }
    # find the ancestors of other nodes and match until you find a shared ancestor between node one and two
    while(TRUE){
        node2 = edgeList[edgeList[,2]==node2,1]
        if(node2 %in% anc) return(node2)
    }
}