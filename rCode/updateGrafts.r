...
# change with a switch to make false so we output results
...
##################################################
xenoTF = '-x' %in% args | '-xp' %in% args | '-px' %in% args
paraTF = '-p' %in% args | '-xp' %in% args | '-px' %in% args
...
if(length(args) == 3){
    ...
    saveloc = '../outputs/updatedTreeGrafterPoint.out'
}
...
for(i in 1:length(fullpnames)){
    while(fullpnames[[unqspec[i]]][length(fullpnames[[unqspec[i]]])] != 1){
            fullpnames[[unqspec[i]]] = c(fullpnames[[unqspec[i]]],cp[match(fullpnames[[unqspec[i]]][length(fullpnames[[unqspec[i]]])],cp[,1]),2])
        }
...
print(str_c('A total of ',length(unqpthr),' unique PANTHER trees in graft sets'))
for(i in 1:length(unique(unqpthr))){ 
        
    #if(i %% 10 == 0) print(paste(placementcounting,errorcounting,rooterror,lucaerror))
    if(i %% 20 == 0) print(paste('On PANTHER tree number : ',i,' out of ',length(unique(unqpthr)),' trees'))
    if(i < 2) print(paste('On PANTHER tree number : ',i,' out of ',length(unique(unqpthr)),' trees'))
    if(i == length(unqpthr)) print(paste('On PANTHER tree number : ',i,' out of ',length(unique(unqpthr)),' trees'))
    ...
    for(j in 1:length(graftAN)){
        ...
        orthoAN = newAN[idNum[j]]
        if(newAN[idNum[j]] %in% anlist){ # deal with case we are bound to leaf node by finding pareant AN number
            mAN = match(newAN[idNum[j]],anlist)
            orthoAN = tree$tree$node.label[tree$tree$edge[match(mAN,tree$tree$edge[,2]),1] - tree$tree$edge[1,1] + 1] 
            addBackLeaf = TRUE
        }
        oposition = match(orthoAN,tree$tree$node.label) # position in the list of node labels for graft
        ...
        if(leafOrtho[idNum[j]] != 0 || addBackLeaf) orthos[[idNum[j]]] = c(orthos[[idNum[j]]],uniprot[leafOrtho[idNum[j]]])
...
if(xenoTF) results = cbind(results,xenosList)
if(paraTF) results = cbind(results,parasList)





write.table(results,saveloc,sep='\t') 
